#include "dicutils.h"
#include "local_struct_queue_roi_separate.h"
#include "local_struct_roi_overall.h"
#include "local_struct_roi_separate.h"
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <numeric>
...
// THREAD SAFE
double dicutils::ncorr_round(const double &r) {
...
// THREAD SAFE
int dicutils::sign(const double &r) {
    if (r > 0) {
        return 1;
    } else if (r < 0) {
        return -1;
...
// THREAD SAFE
int dicutils::mod_pos(const int &i, const int &n) {
    return (i % n + n) % n;
}

// THREAD SAFE
void dicutils::form_boundary(std::vector<std::vector<int>> &vec_boundary, const std::vector<int> &point_init, const class_logical_array &mask, int &direc) {
    // Initialize newpoint
...
// THREAD SAFE
void dicutils::cholesky(std::vector<double> &mat, bool &positivedef, const int &size_mat) {
...
// THREAD SAFE
void dicutils::forwardsub(std::vector<double> &vec, const std::vector<double> &mat, const int &size_mat) {
...
// THREAD SAFE
void dicutils::backwardsub(std::vector<double> &vec, const std::vector<double> &mat, const int &size_mat) {
...
// NOT THREAD SAFE
bool dicutils::interp_qbs(double &interp, const double &x_tilda, const double &y_tilda, const class_double_array &plot_interp, const class_logical_array &mask, const int &offset_x, const int &offset_y, const int &border_bcoef) {
...
// THREADSAFE
void dicutils::expand_filt(class_double_array &plot_extrap, const class_inverseregion &inverseregion) {
...
// THREAD SAFE
void dicutils::form_regions(std::vector<vec_struct_region> &region, bool &removed, const class_logical_array &mask, const int &cutoff, const bool &preservelength) {
...
double dicutils::ncc(const std::vector<double> &f, const std::vector<double> &g) {
...
double dicutils::ncc(const std::vector<double> &f,
